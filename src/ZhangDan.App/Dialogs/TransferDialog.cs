...
internal sealed class TransferDialog : Window
{
    ...
    private readonly TextBox _actualBox = new() { Width = 300 };
    ...
    public TransferDialog(LedgerSession ledger, DateTime defaultDate, AppSettings settings, TransferEditable? edit = null)
    {
        ...
        else
        {
            ...
            _actualBox.Text = ((edit.PrincipalCents + edit.DeltaCents) / 100m).ToString("0.##", CultureInfo.InvariantCulture);
        ...
        panel.Children.Add(FieldRow("实际到账(元)", _actualBox));
    ...
    private void TryAccept()
    {
        ...
        if (!ParseMoney(_actualBox.Text, out var actual) || actual < 0)
        {
            _error.Text = "实际到账请填数字(元)。";
            return;
        }

        PrincipalCents = Money.ToCents(principal);
        DeltaCents = Money.ToCents(actual) - PrincipalCents;
        DialogResult = true;
    }
}
